...
Model* OurSystem::build_model() {
    Model * model = new Model(PHRASE_BASED);
    model->Init();
    return model;
}
...
char* OurSystem::Decoding(const char * infn, Model * model, int decoderType, bool serviceMode)
{
   OurSystem::verbose = ConfigManager.GetInt("verbose", 0);
